...
const DashboardCard = ({ product }: DashboardCardProps) => {
  ...
  const availableColors = Array.from(new Map(variants.map((v) => [v.colorName, v])).values());
  const availableSizes = selectedColor ? variants.filter((v) => v.colorName === selectedColor) : [];
  ...
  return (
    <Card
      hoverable
      className={
        'w-full h-auto rounded-xl border border-gray-200 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group'
      }
      cover={
        <div
          className={
            'relative bg-gradient-to-br from-gray-50 to-white flex justify-center items-center p-4 overflow-hidden'
          }
        >
          <Image
            alt={title}
            className={
              'w-full h-[220px] object-contain transform group-hover:scale-110 transition-all duration-500 ease-out'
            }
            height={220}
          ...
          {stock > 0 && stock < 1000 && selectedVariant ? (
            <div
              className={
                'absolute top-3 right-3 bg-gradient-to-r from-orange-500 to-red-500 text-white text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg animate-pulse'
              }
            >
              Only {stock} left!
            </div>
          ...
          {selectedColor ? (
            <div
              className={
                'absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-gray-800 text-xs font-medium px-3 py-1.5 rounded-full shadow-md border border-gray-200'
              }
            >
    ...
    >
      <div className={'space-y-4'}>
        <h2
          className={
            'text-gray-800 text-base font-semibold leading-snug line-clamp-2 group-hover:text-[#007BFF] transition-colors duration-200'
          }
        >
          {title}
        </h2>
            ...
            {availableColors.map((color) => (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (firstSizeForColor) setSelectedSize(firstSizeForColor.size);
                  setQuantity(1);
                }}
              ...
              >
                ...
                {selectedColor === color.colorName ? (
                  <div
                    className={'absolute inset-0 bg-[#007BFF]/20 flex items-center justify-center'}
                  >
        ...
        >
          <div
            className={
              'flex items-center bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-1 border-2 border-gray-200 shadow-sm hover:shadow-md transition-all duration-300'
            }
          >
            <Button
              className={
                'border-none shadow-none h-9 w-9 text-gray-700 hover:text-[#007BFF] bg-transparent hover:bg-white hover:scale-110 rounded-md transition-all duration-200 disabled:opacity-40'
              }
              disabled={quantity <= 1}
              icon={<MinusOutlined />}
              onClick={decrease}
          ...
          >
            {!selectedVariant ? 'Select Variant' : stock === 0 ? 'Out of Stock' : 'Add to Cart'}
          </Button>
        </div>
      </div>
    </Card>
